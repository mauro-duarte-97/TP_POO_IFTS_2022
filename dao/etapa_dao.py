...
class Etapa_DAO(DAO):
    ...
    def modificar_registro(self, objeto):
        try:
            db, cursor = self.conectar_bd()
            cursor.execute(f"UPDATE {self.nombre_tabla} SET etapas='{objeto.etapas}' WHERE nombre='{objeto.etapas}'", )
            # Guardar (commit) los cambios
            db.commit()
            print(f"La etapa {objeto.nombre} se ha actualizado correctamente")
        except Exception as e:
            print(f"Ocurrió un error la etapa {objeto.nombre}. {e}")
        finally:
            db.close()
