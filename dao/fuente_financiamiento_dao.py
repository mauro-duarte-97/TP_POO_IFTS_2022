...
class FuenteFinanciamiento_DAO(DAO):
    ...
    def modificar_registro(self, objeto):
        try:
            db, cursor = self.conectar_bd()
            cursor.execute(f"UPDATE {self.nombre_tabla} SET fuentes_finaciamiento='{objeto.fuentes_financiamiento}' WHERE nombre='{objeto.nombre}'", )
            # Guardar (commit) los cambios
            db.commit()
            print(f"La fuente de financiamiento {objeto.nombre} se ha actualizado correctamente")
        except Exception as e:
            print(f"Ocurrió un error al modificar ls fuente de financiamiento {objeto.nombre}. {e}")
        finally:
            db.close()
