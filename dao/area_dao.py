...
class Area_DAO(DAO):
    ...
    def modificar_registro(self, objeto):
        ...
        try:
            db, cursor = self.conectar_bd()
            cursor.execute(f"UPDATE {self.nombre_tabla} SET area='{objeto.AREA}' WHERE area='{objeto.AREA}'", )
            # Guardar (commit) los cambios
            db.commit()
            print(f"El area correspondiente {objeto.AREA} se ha actualizado correctamente")
        except Exception as e:
            print(f"Ocurrió un error al modificar el area correspondiente  {objeto.AREA}. {e}")
        finally:
            db.close()
