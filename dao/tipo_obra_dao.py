...
class TipoObra_DAO(DAO):
    ...
    def modificar_registro(self, objeto):
        try:
            db, cursor = self.conectar_bd()
            cursor.execute(f"UPDATE {self.nombre_tabla} SET tipos_obras='{objeto.tipos_obras}' WHERE nombre='{objeto.nombre}'", )
            # Guardar (commit) los cambios
            db.commit()
            print(f"El tipo de obra {objeto.nombre} se ha actualizado correctamente")
        except Exception as e:
            print(f"Ocurrió un error al modificar el tipo de obra {objeto.nombre}. {e}")
        finally:
            db.close()
