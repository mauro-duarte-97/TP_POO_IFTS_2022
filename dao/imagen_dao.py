...
class Imagen_DAO(DAO):
    ...
    def modificar_registro(self, objeto):
        try:
            db, cursor = self.conectar_bd()
            cursor.execute(f"UPDATE {self.nombre_tabla} SET imagenes_obras='{objeto.imagenes_obras}' WHERE nombre='{objeto.nombre}'", )
            # Guardar (commit) los cambios
            db.commit()
            print(f"Las imagenes {objeto.nombre} se ha actualizado correctamente")
        except Exception as e:
            print(f"Ocurrió un error al cargar la imagen {objeto.nombre}. {e}")
        finally:
            db.close()
